...
class Lecteur(tkinter.Frame):
    def __init__(self, master, parametres):
        ...
        #Titre
        resultatLabel = tkinter.Label(self, text="Résultat de la génération", font = self.titre, height = hauteurBout, bg='white').grid(row = 0, column = 0, sticky="W") 
    ...
    def nextSong(self):
        if(len(self.listeMusiques) == 0):
            return
        if self.lecteurState == "Play": #Si c'est 1 c'est qu'un son est joué
