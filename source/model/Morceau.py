...
class Morceau:
    ...
    def getFormat(self, format_name):
        # renvoie l'encodage du fichier dans le format spécifié (s'il existe)
        if format_name not in self.format_liste:
            print("Erreur : le format {} n'est pas supporte !".format(format_name))

        filenames = []
        files_content = []

        if format_name == "Rythme":
            content, ensemble_elements = self.preparer_track_rythme()
            for index in range(len(content)):
                if content[index] != '':
                    filenames.append(self.filename.replace(".csv", "")+"-"+str(index+1)+".format")
            ...
            content, ensemble_elements = self.preparer_track_melodie()  # on récupère toutes les pistes du morceau
    ...
    # Méthodes pour le format MELODIE ==========================
